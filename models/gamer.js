...
const Gamer = function (model) {
  ...
  model.login = async function (session, credentials) {
    ...
    const gamers = await model.findByFields({ username: user }).catch((err) => {
      throw new Error(err);
    });
  ...
  model.currentUser = async function (session) {
    if (session.user) {
      const gamer = await model.findById(session.user.id).catch((err) => {
        throw new Error(err);
      });
      return gamer;
    } else {
  ...
  model.games = async function (id) {
    ...
    const games = await Games.findAll().catch((err) => {
      throw new Error(err);
    });
    ...
    games.forEach(function (gamerecord) {
      ...
      if (!gameUtils.tournamentComplete(gameDetails.start, gameDetails.end)) {
