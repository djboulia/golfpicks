...
@Component({
  selector: 'app-input-field-numeric',
...
export class InputFieldNumericComponent {
  @Input() id?: string = '';
  @Input() name?: string = '';
  ...
  onInput(event: Event) {
    ...
    console.log('numeric input: ', input.value, input.valueAsNumber);
    if (Number.isFinite(Number(input.value))) {
      console.log('valid number input: ', input.value);
      this.error = false;
      this.hint = undefined;
      this.valueChange.emit(input.value ? Number(input.value) : undefined);
    } else {
      console.log('INVALID number input: ', input.value);
      this.error = true;
      this.hint = 'Please enter a valid number';
    }
  }
}
